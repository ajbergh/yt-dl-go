...
`feat/windows-data-dir-acl` is active. Startup now reads the Windows DATA_DIR owner and DACL, requires ownership by the current user, and rejects grants to common shared logon groups and package-wide application principals; SYSTEM and Administrators remain allowed. The existing POSIX 0700 check remains in place. PR #75 first exposed Windows test folders owned by Administrators and the existing `TestCancellationQueueAndTimeout` 45-second wait; fixtures now set owner plus a private DACL and the scoped wait is 90 seconds. Its rerun passed all platform package jobs but found that `TestRetentionCleansEmptyFailedAndCancelledJobs` could change `done_at` before the final asynchronous persistence save completed. The test now waits for the durable terminal row before aging it and advances the prune clock for deterministic expiry. The affected test passed 20 consecutive local runs, followed by the full Go suite, `go vet ./...`, and Linux cross-compilation; CI is rerunning these fixes. Library export/import remains open.
