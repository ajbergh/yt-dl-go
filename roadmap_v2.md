...
`feat/windows-data-dir-acl` is active. Startup now reads the Windows DATA_DIR owner and DACL, requires ownership by the current user, and rejects grants to common shared logon groups and package-wide application principals; SYSTEM and Administrators remain allowed. The existing POSIX 0700 check remains in place. PR #75 exposed two CI fixture issues: Windows runner temp folders have an administrative owner, and `TestCancellationQueueAndTimeout` again reached its 45-second bound under runner load. Test fixtures now set the owner as well as a private DACL, and the scoped wait is 90 seconds. Windows/Linux/macOS CI is rerunning; local Windows tests and vet pass. Library export/import remains open.
