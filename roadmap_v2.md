...
**Progress (2026-09-24):** The storage foundation merged in [PR #51](https://github.com/ajbergh/yt-dl-go/pull/51) as `64c501b`. It adds a durable per-file `library_items` table with job/item provenance, migration backfill (including grouped chapter files), same-transaction mirroring on job saves, and explicit history deletion cleanup. The table is independent of `jobs` and has no cascading foreign key. CI exposed excess repeated mirror work on large playlists; the follow-up skips unchanged Library rows and cleans stale rows only when file state changes. Final CI passed Go/race and source validation, lint/static analysis, CodeQL, and Linux, Windows, and macOS package/build checks. [PR #53](https://github.com/ajbergh/yt-dl-go/pull/53) (`7d3fce5`) adds `GET /api/library` as a direct `library_items` read and switches Library cards to it; job actions remain job-scoped. All PR checks passed, including frontend integration/E2E, Go/race/source validation, CodeQL, and Linux, Windows, and macOS builds. This read path builds Library cards without consulting the in-memory job map or `job_files`, but Library rows still follow job-history deletion/retention. [PR #62](https://github.com/ajbergh/yt-dl-go/pull/62) merged as `87b4965`: migration v24 backfills and transactionally maintains compact `library_sources` metadata, and Library reads/facets use it instead of joining job history. [PR #67](https://github.com/ajbergh/yt-dl-go/pull/67) is implementing the next lifecycle slice: startup loads active jobs only, terminal state is evicted after its final event, terminal actions hydrate a single saved job on demand, and retention queries only expired candidate IDs before loading records one at a time. CI review of the in-flight implementation found legacy checks expecting completed jobs in the active in-memory map; those were adapted to read durable records and Library chapters from `/api/library`. The latest CI run exposed that per-file `MediaType` was not stored in `job_files`, so evicted audio-job hydration lost the format discriminator even though the Library JSON retained it; hydration now restores the existing audio-job value for primary and additional files. Lint flagged a queue-loading wrapper made obsolete by the active and single-job query paths; it has been removed. All Linux/macOS package targets and Go analysis pass on the latest run, and source validation passed Go tests, formatting, lint, race, and vet checks. Its browser E2E then exposed an assertion still reading completed history from `/api/jobs`; the script now checks durable completion, restart persistence, and deletion through `/api/library`. The final check set on commit `34d4c34` passed Go/race/source validation, frontend integration and browser E2E, lint/vet, CodeQL, and Linux, Windows, and macOS builds. PR #67 is open and mergeable; independent file ownership, separate deletion semantics, and 10,000-item scale validation remain open.
