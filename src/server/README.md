...
`best`, `2160`, `1440`, `1080`, `720`, and `480` are maximum heights. Through 1080p, the selector preserves the compatibility-oriented adaptive H.264 + AAC MP4 path when it improves on an available progressive stream. For higher ceilings, it can select adaptive VP9 or AV1 WebM video with Opus audio and remux the tracks to WebM without transcoding. At equal resolution and frame rate VP9 is preferred over AV1 for broader decoder compatibility; AV1 remains supported when it is the better or only high-resolution representation. Otherwise the service uses the best compatible progressive MP4 or WebM stream within the requested ceiling. Audio-only jobs select a compatible AAC-in-MP4 stream. `audioFormat:"m4a"` preserves that stream directly with no lossy generation; `audioFormat:"mp3"` converts it at the requested bitrate using the in-process Go AAC decoder and MP3 encoder. Other source audio codecs are not currently selected.
...
| `NO_BROWSER` | unset | Set to `1` to suppress automatic OS browser launch for CI/headless startup |
